...
class YAMLCacheManager:
    ...
    def __init__(self, max_cache_size: int = 1000, enable_hot_reload: bool = None):
        ...
        logger.debug(f"🐛 🚀 YAML Cache Manager initialized (max_size={max_cache_size}, hot_reload={self._enable_hot_reload})")
    
    def get_yaml(self, file_path: str, force_reload: bool = False) -> Optional[Dict[str, Any]]:
        """
        ...
        with self._lock:
            ...
            if os.path.exists(pattern_dir):
                current_dir_mtime = max(
                    ...
                    max((os.path.getmtime(os.path.join(pattern_dir, item)) 
                         for item in os.listdir(pattern_dir) 
                         if os.path.isdir(os.path.join(pattern_dir, item))), default=0)
                )
            else:
                current_dir_mtime = 0
    ...
    def get_agent_team_mapping(self, agent_id: str, force_reload: bool = False) -> Optional[str]:
        """
...
_yaml_cache_manager: Optional[YAMLCacheManager] = None
...
def load_yaml_cached(file_path: str, force_reload: bool = False) -> Optional[Dict[str, Any]]:
    """
...
def get_agent_team_cached(agent_id: str, force_reload: bool = False) -> Optional[str]:
    """
