    ...
    def _extract_agno_native_metrics(self, response: Any) -> Dict[str, Any]:
        ...
        if hasattr(response, 'session_metrics') and response.session_metrics:
            ...
            metrics.update({
                ...
                'completion_tokens': getattr(session_metrics, 'completion_tokens', 0),
                'audio_tokens': getattr(session_metrics, 'audio_tokens', 0),
                'input_audio_tokens': getattr(session_metrics, 'input_audio_tokens', 0),
                'output_audio_tokens': getattr(session_metrics, 'output_audio_tokens', 0),
                'cached_tokens': getattr(session_metrics, 'cached_tokens', 0),
                'cache_write_tokens': getattr(session_metrics, 'cache_write_tokens', 0),
                'reasoning_tokens': getattr(session_metrics, 'reasoning_tokens', 0),
                'time': getattr(session_metrics, 'time', 0.0),
                'time_to_first_token': getattr(session_metrics, 'time_to_first_token', 0.0),
            })
            
            # Extract optional detailed metrics
            if hasattr(session_metrics, 'prompt_tokens_details') and session_metrics.prompt_tokens_details:
                metrics['prompt_tokens_details'] = session_metrics.prompt_tokens_details
                
            if hasattr(session_metrics, 'completion_tokens_details') and session_metrics.completion_tokens_details:
                metrics['completion_tokens_details'] = session_metrics.completion_tokens_details
            ...
            if isinstance(run_metrics, dict):
                ...
                for metric_name, metric_values in run_metrics.items():
                    if isinstance(metric_values, list) and metric_values:
                        ...
                        if metric_name.endswith('_tokens') or metric_name in ['time', 'time_to_first_token']:
                            metrics[metric_name] = sum(metric_values) if all(isinstance(v, (int, float)) for v in metric_values) else metric_values[-1]
                        else:
    ...
    def get_metrics_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "capabilities": {
                ...
                "detailed_metrics": ["prompt_tokens_details", "completion_tokens_details"],
                "additional_metrics": ["additional_metrics", "model", "response_length"],
