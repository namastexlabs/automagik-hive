...
class TestSettings:
    ...
    def test_settings_initialization_with_defaults(self, temp_project_dir, mock_env_vars, clean_singleton):
        """Test settings initialization with default values."""
        with patch('lib.config.settings.Path', return_value=temp_project_dir / "lib" / "config" / "settings.py"):
            ...
            assert test_settings.app_name == "PagBank Multi-Agent System"
            assert test_settings.version == "0.1.0"
            assert test_settings.environment == "test"  # From mock_env_vars
            
            # Test API settings
            assert test_settings.log_level == "DEBUG"  # From mock_env_vars
            assert "%(asctime)s - %(name)s - %(levelname)s - %(message)s" in test_settings.log_format
    ...
    def test_settings_directory_creation(self, temp_project_dir, mock_env_vars):
        ...
        with patch('lib.config.settings.Path.__file__', str(temp_project_dir / "lib" / "config" / "settings.py")):
            test_settings = Settings()
    ...
    def test_settings_environment_variable_parsing(self, mock_env_vars, clean_singleton):
    ...
    def test_settings_metrics_configuration_validation(self, mock_env_vars, mock_logger, clean_singleton):
    ...
    def test_settings_metrics_configuration_invalid_values(self, mock_invalid_env_vars, mock_logger, clean_singleton):
        """Test metrics configuration with invalid values uses defaults."""
        test_settings = Settings()
        
        # Should use secure defaults when invalid values provided
...
class TestSettingsUtilityFunctions:
    ...
    def test_get_project_root_function(self, temp_project_dir, clean_singleton):
        ...
        with patch('lib.config.settings.Path.__file__', str(temp_project_dir / "lib" / "config" / "settings.py")):
            ...
            with patch('lib.config.settings.settings', Settings()):
...
class TestSettingsEdgeCases:
    ...
    def test_settings_with_missing_logger_import(self, clean_singleton):
        ...
        with patch.dict(os.environ, {
            'HIVE_METRICS_BATCH_SIZE': 'invalid_number'
        }):
            # Mock logger import failure
            with patch('lib.config.settings.logger', side_effect=ImportError("Logger not available")):
                test_settings = Settings()
