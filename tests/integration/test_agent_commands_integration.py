                    ...
                    with patch("time.sleep"):
                        ...
                        with patch("pathlib.Path.exists") as mock_exists:
                            ...
                            def exists_side_effect(path_self):
                                ...
                                if ".env.agent" in path_str:
                                    return True
                                elif ".venv" in path_str:
                                    return True
                                elif "logs/agent-server.pid" in path_str:
                                    return True
                                elif "logs/agent-server.log" in path_str:
                                    return True
                                return False
    ...
    def test_agent_serve_parity(self, temp_workspace_with_makefile):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            make_result = subprocess.run([
                "make", "agent"
            ], cwd=temp_workspace_with_makefile, capture_output=True, text=True)
