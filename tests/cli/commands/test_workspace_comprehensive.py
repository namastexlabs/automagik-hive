...
class TestWorkspaceCommandsInitialization:
    ...
    def test_string_path_conversion(self):
        ...
        # WorkspaceCommands expects Path objects, not strings
        with pytest.raises(TypeError):
            # Should fail - string input not supported in type hints
        ...
        for param in test_params:
            try:
                if param is not None:
                    ...
                    # None parameter might cause TypeError
                    with pytest.raises(TypeError):
                        workspace_cmd.start_workspace(param)
            except Exception as e:
                if param is None:
