...
class TestWorkspaceManagerInitialization:
    ...
    def test_init_with_different_working_directory(self):
        ...
        test_paths = [Path('/home/user/project'), Path('/var/lib/app'), Path('/tmp/workspace')]
...
class TestWorkspaceServerStartup:
    ...
    def test_start_server_success(self, temp_workspace):
        ...
        with patch('os.chdir') as mock_chdir, \
            ...
            assert result is True
            mock_chdir.assert_called_once_with(temp_workspace)
            mock_run.assert_called_once_with(["python", "-m", "api.main"])

    def test_start_server_workspace_not_found(self, temp_workspace):
        """Test server startup fails when workspace directory doesn't exist."""
        manager = WorkspaceManager()
        nonexistent_path = str(temp_workspace / "nonexistent")
    ...
    def test_start_server_api_file_missing(self, temp_workspace):
        ...
        with patch('builtins.print') as mock_print:
            ...
            assert result is False
            expected_api_file = temp_workspace / "api" / "main.py"
...
class TestWorkspaceCommandExecution:
    ...
    def test_run_command_success_with_capture(self, temp_workspace):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            mock_run.assert_called_once_with(
                ["echo", "test"], cwd=None, capture_output=True, text=True, check=True
            )
    ...
    def test_run_command_with_working_directory(self, temp_workspace):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            mock_run.assert_called_once_with(
                ["ls"], cwd=temp_workspace, capture_output=True, text=True, check=True
            )
