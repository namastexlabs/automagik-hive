...
class TestComponentVersionService:
    ...
    def test_service_method_signatures(self):
        ...
        assert callable(getattr(service, '_get_db_service'))
        assert callable(getattr(service, 'close'))
        assert callable(getattr(service, 'create_component_version'))
        assert callable(getattr(service, 'get_component_version'))
        assert callable(getattr(service, 'get_active_version'))
        assert callable(getattr(service, 'set_active_version'))
        assert callable(getattr(service, 'list_component_versions'))
        assert callable(getattr(service, 'add_version_history'))
        assert callable(getattr(service, 'get_version_history'))
...
class TestComponentVersionServiceEdgeCases:
    ...
    def test_dataclass_equality(self):
        ...
        version1 = ComponentVersion(
            id=1, component_id="test", component_type="agent", version=1,
            config={}, description="test", is_active=True, created_at=dt, created_by="user"
        )
        ...
        version3 = ComponentVersion(
            id=2, component_id="test", component_type="agent", version=1,
            config={}, description="test", is_active=True, created_at=dt, created_by="user"
        )
    ...
    def test_version_history_equality(self):
        ...
        history1 = VersionHistory(
            id=1, component_id="test", from_version=1, to_version=2,
            action="update", description="test", changed_by="user", changed_at=dt
        )
    ...
    def test_dataclass_repr(self):
        ...
        version = ComponentVersion(
            id=1, component_id="test", component_type="agent", version=1,
            config={}, description="test", is_active=True, 
        ...
        history = VersionHistory(
            id=1, component_id="test", from_version=1, to_version=2,
            action="update", description="test", changed_by="user", changed_at=datetime.now()
        )
