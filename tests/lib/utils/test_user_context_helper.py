...
class TestCreateUserContextState(unittest.TestCase):
    ...
    def test_creates_session_state_with_all_fields(self):
        ...
        result = user_context_helper.create_user_context_state(
            user_id="42", user_name="Alice Silva", phone_number="11987654321", cpf="12345678900"
        )
    ...
    def test_handles_partial_user_data(self):
        ...
        result = user_context_helper.create_user_context_state(user_id="99", user_name="Bob Santos")

        expected = {"user_context": {"user_id": "99", "user_name": "Bob Santos"}}
        self.assertEqual(result, expected)

    def test_filters_none_values(self):
        """Verify that None values are properly filtered out from session state."""
        result = user_context_helper.create_user_context_state(
            user_id=None, user_name="Charlie", phone_number=None, cpf="98765432100"
        )

        expected = {"user_context": {"user_name": "Charlie", "cpf": "98765432100"}}
        self.assertEqual(result, expected)

    def test_accepts_custom_kwargs(self):
        """Test that additional custom fields can be passed via kwargs."""
        result = user_context_helper.create_user_context_state(
            user_id="7", user_name="Diana", role="admin", department="engineering"
        )

        expected = {
            "user_context": {"user_id": "7", "user_name": "Diana", "role": "admin", "department": "engineering"}
        }
        self.assertEqual(result, expected)

    def test_filters_none_from_kwargs(self):
        """Ensure None values in kwargs are also filtered out."""
        result = user_context_helper.create_user_context_state(user_id="10", custom_field=None, valid_field="value")

        expected = {"user_context": {"user_id": "10", "valid_field": "value"}}
        self.assertEqual(result, expected)
    ...
    def test_logs_creation_with_data(self, mock_logger):
        ...
        user_context_helper.create_user_context_state(user_id="123", user_name="Test User")
...
class TestGetUserContextFromAgent(unittest.TestCase):
    ...
    def test_extracts_context_from_valid_agent(self):
        ...
        agent = Mock()
        agent.session_state = {"user_context": {"user_id": "55", "user_name": "Eduardo"}}

        result = user_context_helper.get_user_context_from_agent(agent)

        expected = {"user_id": "55", "user_name": "Eduardo"}
        self.assertEqual(result, expected)
