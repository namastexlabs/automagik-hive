...
class TestBusinessUnitDetection:
    ...
    def test_detect_business_unit_keyword_boundaries(self, filter_with_test_config):
        ...
        test_cases = [
            ...
            ("I need credit for my payment", "payments"),  # "credit" matches cards, "payment" matches payments, but more payment context
...
class TestDocumentFiltering:
    ...
    def test_filter_documents_without_content_attribute(self, filter_for_document_tests):
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_initialization_with_malformed_config(self):
        """Test initialization with malformed configuration data."""
        malformed_config = {
            "business_units": {
                "unit1": {
                    "name": "Valid Unit",
                ...
                "unit4": None  # Null unit
            },
            "search_config": "not-a-dict",  # Invalid type
            "performance": None  # Null performance
        }
        ...
        with patch('lib.knowledge.config_aware_filter.load_global_knowledge_config') as mock_load:
            ...
            # Should still work with valid parts
            assert "unit1" in filter_instance.business_unit_keywords
            valid_unit = filter_instance.business_unit_keywords["unit1"]
            assert valid_unit["name"] == "Valid Unit"
            assert valid_unit["keywords"] == ["valid"]
    ...
    def test_document_filtering_with_exception_in_detection(self, filter_for_document_tests):
        """Test document filtering when detection raises exception."""
        doc = Mock()
        doc.meta_data = {}
        doc.content = "Some content"
        
        with patch.object(filter_for_document_tests, 'detect_business_unit_from_text', side_effect=Exception("Detection error")):
            # Should handle exceptions gracefully
            filtered = filter_for_document_tests.filter_documents_by_business_unit([doc], "payments")
            
            # Should return empty list when detection fails
            assert filtered == []
