...
class TestVersionFactory:
    ...
    def test_version_factory_component_loading(self, mock_storage, temp_directory):
        ...
        with patch("lib.utils.version_factory.PostgresStorage") as mock_postgres:
            ...
            with patch(
                "lib.utils.version_factory.discover_components_cached",
            ) as mock_discover:
...
class TestVersionFactoryUtilityFunctions:
    ...
    def test_environment_variable_handling(self):
        ...
        with (
            patch.dict(
                "os.environ",
                {"HIVE_DATABASE_URL": "postgresql://test:5432/db"},
            ),
            patch("lib.utils.version_factory.PostgresStorage") as mock_storage,
        ):
            VersionFactory()
    ...
    def test_agno_integration(self):
        ...
        from lib.utils.version_factory import Agent, Claude, Team, Workflow
...
class TestVersionFactoryEdgeCases:
    ...
    def test_large_configuration_handling(self):
        ...
        # Create large config
        large_config = {"knowledge": {f"key_{i}": f"value_{i}" for i in range(1000)}}
