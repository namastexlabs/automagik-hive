...
class TestMetricsServicePerformance:
    ...
    async def test_batch_collection_latency(self, mock_metrics_service):
        ...
        assert max_latency < 0.1, f"Max latency {max_latency:.3f}ms exceeds 0.1ms target"
        assert avg_latency < 0.05, f"Avg latency {avg_latency:.3f}ms exceeds 0.05ms target"
    ...
    async def test_concurrent_collection_performance(self, mock_metrics_service):
        ...
        async def collect_metric(agent_id):
            start_time = time.perf_counter()
            result = await service.collect_metrics(f"agent_{agent_id}", "agent", {"id": agent_id})
            end_time = time.perf_counter()
    ...
    async def test_queue_overflow_handling(self, mock_metrics_service):
        ...
        for i in range(150):  # Try to add more than queue size
            result = await service.collect_metrics(f"agent_{i}", "agent", {"overflow": i})
            if result:
    ...
    async def test_background_processing_efficiency(self, mock_metrics_service):
        ...
        assert mock_storage.store_metrics.call_count <= 25  # Should be batched, not 1-to-1
    ...
    async def test_error_recovery_performance(self, mock_metrics_service):
        ...
        start_time = time.perf_counter()
        for i in range(30):
            result = await service.collect_metrics(f"agent_{i}", "agent", {"error_test": i})
        ...
        assert avg_latency < 0.1, f"Error recovery avg latency {avg_latency:.3f}ms too high"
    ...
    async def test_shutdown_performance(self, mock_metrics_service):
        ...
        assert shutdown_time < 200, f"Shutdown took {shutdown_time:.1f}ms, should be <200ms"
    ...
    def test_sync_wrapper_performance(self):
        ...
        with pytest.raises(RuntimeError):  # Expected since no event loop
            result = service.collect_from_response(
        ...
        assert wrapper_time < 1.0, f"Sync wrapper took {wrapper_time:.3f}ms, should be <1ms"
    ...
    async def test_memory_efficiency(self, mock_metrics_service):
        ...
        for batch in range(10):
            for i in range(20):
                await service.collect_metrics(f"batch_{batch}_agent_{i}", "agent", {"data": "x" * 100})
        ...
        assert final_stats["queue_size"] < 50, f"Queue size {final_stats['queue_size']} too large"
