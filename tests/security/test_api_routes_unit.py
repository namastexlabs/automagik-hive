...
def mock_version_service():
    ...
    with patch('lib.services.version_sync_service.get_version_service') as mock:
...
def mock_mcp_manager():
    ...
    with patch('lib.mcp.connection_manager.get_connection_manager') as mock:
        mock_manager = AsyncMock()
        mock.return_value = mock_manager
        yield mock_manager

@pytest.fixture
def test_app(mock_database_service, mock_version_service, mock_mcp_manager):
    ...
    from api.routes.mcp_router import mcp_router
    ...
    app.include_router(version_router, prefix="/api/v1/version")
...
class TestMCPRouterUnit:
    ...
    def test_mcp_status_endpoint(self, client, mock_mcp_manager):
    ...
    def test_mcp_servers_endpoint(self, client, mock_mcp_manager):
...
class TestVersionRouterUnit:
    ...
    @patch('api.routes.version_router.validate_agent_message')
    def test_version_execute_endpoint_validation(self, mock_validate, client, mock_version_service):
        ...
        mock_validate.assert_called_once_with("test message", "versioned component execution")
