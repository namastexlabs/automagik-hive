...
from lib.services.database_service import DatabaseService, get_db_service, close_db_service
...
class TestDatabaseServiceSQLInjectionPrevention:
    ...
    def mock_service(self):
        ...
        with patch.dict(os.environ, {"HIVE_DATABASE_URL": "postgresql://user:pass@host:5432/db"}):
            service = DatabaseService()
    ...
    async def test_fetch_one_with_malicious_parameters(self, mock_service):
        ...
        result = await service.fetch_one(query, malicious_params)
    ...
    async def test_fetch_all_with_malicious_parameters(self, mock_service):
        ...
        result = await service.fetch_all(query, malicious_params)
...
class TestDatabaseServiceErrorHandling:
    ...
    async def test_connection_failure_handling(self, mock_service):
        ...
        with patch('lib.services.database_service.AsyncConnectionPool') as mock_pool_class:
            mock_pool_class.side_effect = Exception("Connection failed: Invalid credentials")
    ...
    async def test_sensitive_data_not_leaked_in_errors(self, mock_service):
        ...
        mock_connection.execute.side_effect = Exception("Authentication failed for user 'secret_user' with password 'secret_pass'")
        mock_service.pool = mock_pool
    ...
    async def test_transaction_rollback_on_error(self, mock_service):
        ...
        mock_pool.connection = mock_connection_context
        mock_connection.transaction = mock_transaction
        mock_connection.execute.side_effect = [None, Exception("Query failed")]  # First succeeds, second fails
...
class TestDatabaseServiceConcurrencySecurity:
    ...
    async def test_concurrent_transaction_isolation(self, mock_service):
        ...
        async def execute_transaction(tx_id):
            operations = [
                (f"INSERT INTO tx_{tx_id} (value) VALUES (%(value)s)", {"value": f"data_{tx_id}"}),
...
class TestGlobalDatabaseServiceSecurity:
    ...
    @pytest.mark.asyncio
    async def test_global_service_initialization_error_handling(self, clean_global_service):
        """Test error handling during global service initialization."""
        with patch.dict(os.environ, {"HIVE_DATABASE_URL": "postgresql://user:pass@host:5432/db"}):
            with patch('lib.services.database_service.DatabaseService') as mock_db_class:
                mock_instance = AsyncMock()
                mock_instance.initialize.side_effect = Exception("Database connection failed")
                mock_db_class.return_value = mock_instance
