...
class TestDatabaseServiceUnit:
    ...
    def test_database_service_url_conversion(self):
        ...
        with patch.dict(os.environ, {"HIVE_DATABASE_URL": "postgresql+psycopg://user:pass@host:5432/db"}):
            service = DatabaseService()
    ...
    async def test_database_service_execute(self):
        ...
        with patch.dict(os.environ, {"HIVE_DATABASE_URL": "postgresql://user:pass@host:5432/db"}):
            ...
            service.get_connection.return_value.__aenter__ = AsyncMock(return_value=mock_connection)
    ...
    async def test_database_service_execute_transaction(self):
        ...
        with patch.dict(os.environ, {"HIVE_DATABASE_URL": "postgresql://user:pass@host:5432/db"}):
            ...
            mock_connection.execute.assert_any_call("INSERT INTO test (name) VALUES (%(name)s)", {"name": "test1"})
            mock_connection.execute.assert_any_call("INSERT INTO test (name) VALUES (%(name)s)", {"name": "test2"})
