...
class TestAuthServiceSecurity:
    ...
    def test_auth_service_initialization_with_key(self, clean_environment, mock_auth_init_service):
        """Test AuthService properly initializes with API key."""
        service = AuthService()
        
        assert service.api_key == "test_key_123"
        assert service.auth_disabled == False
        mock_auth_init_service.ensure_api_key.assert_called_once()

    def test_auth_service_initialization_disabled(self, clean_environment, mock_auth_init_service):
        """Test AuthService respects HIVE_AUTH_DISABLED flag."""
        os.environ["HIVE_AUTH_DISABLED"] = "true"
        
        service = AuthService()
        
        assert service.auth_disabled == True
        assert service.api_key == "test_key_123"

    @pytest.mark.asyncio
    async def test_valid_api_key_acceptance(self, clean_environment, mock_auth_init_service):
    ...
    @pytest.mark.asyncio
    async def test_invalid_api_key_rejection(self, clean_environment, mock_auth_init_service):
    ...
    @pytest.mark.asyncio
    async def test_none_api_key_rejection(self, clean_environment, mock_auth_init_service):
    ...
    @pytest.mark.asyncio
    async def test_empty_string_api_key_rejection(self, clean_environment, mock_auth_init_service):
    ...
    @pytest.mark.asyncio
    async def test_whitespace_api_key_rejection(self, clean_environment, mock_auth_init_service):
    ...
    @pytest.mark.asyncio
    async def test_auth_disabled_bypass(self, clean_environment, mock_auth_init_service):
    ...
    @pytest.mark.asyncio
    async def test_timing_attack_resistance(self, clean_environment, mock_auth_init_service):
    ...
    def test_is_auth_enabled_with_auth_disabled_false(self, clean_environment, mock_auth_init_service):
        """Test is_auth_enabled returns True when auth is enabled."""
        service = AuthService()
        
        assert service.is_auth_enabled() == True

    def test_is_auth_enabled_with_auth_disabled_true(self, clean_environment, mock_auth_init_service):
        """Test is_auth_enabled returns False when auth is disabled."""
        os.environ["HIVE_AUTH_DISABLED"] = "true"
        service = AuthService()
        
        assert service.is_auth_enabled() == False
    ...
    @pytest.mark.asyncio
    async def test_case_sensitive_api_key_validation(self, clean_environment, mock_auth_init_service):
        ...
        for variation in case_variations:
            ...
            assert result == False, f"Case variation {variation} should be rejected"
    ...
    async def test_unicode_and_special_chars_in_key(self, clean_environment):
        ...
        with patch('lib.auth.service.AuthInitService') as mock_init:
            ...
            result = await service.validate_api_key("test_key_!@#$%^&*()_-=+[]{}|;:,.<>!")
            assert result == False
...
class TestAuthServiceEdgeCases:
    ...
    def test_auth_disabled_variations(self, clean_environment):
        ...
        with patch('lib.auth.service.AuthInitService') as mock_init:
            ...
            for value in disabled_values:
                ...
                if value.lower() == "true":
                    assert service.auth_disabled == True, f"Value '{value}' should disable auth"
                else:
                    assert service.auth_disabled == False, f"Value '{value}' should NOT disable auth"
            ...
            for value in enabled_values:
                os.environ["HIVE_AUTH_DISABLED"] = value
                service = AuthService()
                assert service.auth_disabled == False, f"Value '{value}' should keep auth enabled"
    ...
    async def test_concurrent_api_key_validation(self, clean_environment):
        ...
        with patch('lib.auth.service.AuthInitService') as mock_init:
            ...
            keys = ["concurrent_test_key", "wrong_key", "concurrent_test_key", "invalid", "concurrent_test_key"]
