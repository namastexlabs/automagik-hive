...
class TestLoggingContext:
    ...
    def test_filter_creation(self):
        ...
        important_record = logging.LogRecord(
            ...
            msg='This is important', args=(), exc_info=None
        )
        
        unimportant_record = logging.LogRecord(
            name='test', level=logging.INFO, pathname='test.py', lineno=1,
            msg='This is not', args=(), exc_info=None
        )
        
        assert test_filter.filter(important_record) == True
        assert test_filter.filter(unimportant_record) == False
...
class TestPerformanceLogging:
    ...
    def test_conditional_logging(self):
        ...
        # Test isEnabledFor
        assert test_logger.isEnabledFor(logging.ERROR) == True
        assert test_logger.isEnabledFor(logging.DEBUG) == False
        
        # Test conditional logging
        if test_logger.isEnabledFor(logging.DEBUG):
            test_logger.debug('Expensive debug operation: %s', 'expensive_calculation()')
    ...
    def test_lazy_evaluation(self):
        ...
        test_logger.debug('Result: %s', expensive_operation() if test_logger.isEnabledFor(logging.DEBUG) else 'skipped')
