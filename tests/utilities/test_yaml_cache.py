...
from lib.utils.yaml_cache import (
    YAMLCacheManager, CachedYAML, CachedGlob, 
    get_yaml_cache_manager, reset_yaml_cache_manager,
    load_yaml_cached, discover_components_cached, get_agent_team_cached
)
...
class TestYAMLCacheManager:
    ...
    @patch('builtins.open')
    def test_load_yaml_success(self, mock_open, mock_exists, mock_getsize, mock_getmtime):
    ...
    @patch('os.path.exists')
    @patch('builtins.open')
    def test_load_yaml_cache_invalidation(self, mock_open, mock_exists, mock_getsize, mock_getmtime):
    ...
    @patch('glob.glob')
    @patch('os.path.getmtime')
    def test_discover_yaml_files_success(self, mock_getmtime, mock_glob):
        ...
        assert cached_item.pattern == "*.yaml"
        
    @patch('os.path.getmtime')
    def test_discover_yaml_files_cache_hit(self, mock_getmtime):
    ...
    @patch('glob.glob')
    @patch('os.path.getmtime')
    def test_discover_yaml_files_cache_invalidation(self, mock_getmtime, mock_glob):
    ...
    def test_clear_cache(self):
        ...
        # Populate caches
        self.cache._yaml_cache["/test/file.yaml"] = CachedYAML({"test": "data"}, 123.0, "/test/file.yaml", 100)
        self.cache._glob_cache["/test/*.yaml"] = CachedGlob(["/test/file.yaml"], 123.0, "*.yaml")
    ...
    def test_get_cache_stats(self):
        ...
        # Populate caches
        self.cache._yaml_cache["/test/file1.yaml"] = CachedYAML({"test": "data1"}, 123.0, "/test/file1.yaml", 100)
        self.cache._yaml_cache["/test/file2.yaml"] = CachedYAML({"test": "data2"}, 124.0, "/test/file2.yaml", 200)
        self.cache._glob_cache["/test/*.yaml"] = CachedGlob(["/test/file1.yaml"], 123.0, "*.yaml")
        
        stats = self.cache.get_cache_stats()
        
        assert stats["yaml_cache_entries"] == 2
        assert stats["glob_cache_entries"] == 1
        assert stats["total_yaml_size_bytes"] == 300
        assert len(stats["cached_files"]) == 2
        assert len(stats["cached_patterns"]) == 1
    ...
    def test_thread_safety(self):
        ...
        def worker(thread_id):
            try:
                ...
                for i in range(10):
                    key = f"/test/file_{thread_id}_{i}.yaml"
                    content = {"thread": thread_id, "iteration": i}
                    
                    # Add to cache
                    cached_item = CachedYAML(content, time.time(), key, 100)
                    self.cache._yaml_cache[key] = cached_item
                    
                    # Read from cache
                    retrieved = self.cache._yaml_cache.get(key)
                    if retrieved:
                        results.append((thread_id, i, retrieved.content))
                        
                    # Clear specific entry
                    if key in self.cache._yaml_cache:
                        del self.cache._yaml_cache[key]
...
class TestYAMLCacheIntegration:
    ...
    def test_performance_characteristics(self, temp_dir, sample_yaml_config, performance_helper):
        ...
        performance_helper.assert_performance_threshold(second_load_time, 0.001)  # < 1ms for cache hit