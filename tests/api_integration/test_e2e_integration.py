...
class TestE2EUserJourneys:
    ...
    def test_mcp_system_integration_flow(self, test_client, api_headers):
        ...
        if status_response.status_code == status.HTTP_200_OK:
            ...
            config_response = test_client.get(
                "/api/v1/mcp/config",
                headers=api_headers
            )
...
class TestE2EAsyncIntegration:
    ...
    async def test_async_endpoint_integration(self, async_client: AsyncClient, api_headers):
    ...
    async def test_async_concurrent_requests(self, async_client: AsyncClient, api_headers):
