...
from lib.auth.dependencies import auth_service, require_api_key, optional_api_key
...
class AuthTestHelpers:
    ...
    async def assert_auth_success(client, endpoint, headers, method="GET", **kwargs):
        ...
        assert response.status_code != 401, f"Authentication failed with valid key"
        return response
...
def auth_test_scenarios():
    ...
    return {
        "valid_scenarios": [
            ...
            {"key": "hive_key_with_special_!@#$%^&*()", "description": "Key with special chars"}
        ],
        "invalid_scenarios": [
