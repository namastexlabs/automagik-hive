...
def main():
    ...
    try:
        ...
        # ENHANCED SECURITY: Allow tests/ and genie/ directories only
        allowed_prefixes = ["tests/", "genie/"]
        is_allowed_path = any(str(relative_path).startswith(prefix) for prefix in allowed_prefixes)
        
        if not is_allowed_path:
            # BLOCK: Testing agent trying to modify source code outside allowed directories
            error_message = f"""
