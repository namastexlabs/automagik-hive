...
```bash
# Check if backup exists (runtime-generated, see template at backup-info.json.template)
cat .genie/state/backup-info.json 2>/dev/null
```
...
const backupInfo = JSON.parse(await readFile('.genie/state/backup-info.json'));
...
Output: .genie/state/restoration-report.md`
  })
});
...
if (backupStatus === 'completed') {
  // Read restoration report (runtime-generated by backup-analyzer, see restoration-report.md.template)
  const restorationReport = await readFile('.genie/state/restoration-report.md');
