"""
Code Editing Agent Tools - Symbol-Aware Modifications and Code Transformations

Extracted and adapted from Serena's code modification capabilities for multi-agent architecture.
...
def replace_symbol_body(
    ...
    try:
        ...
        return f"Error replacing symbol body: {str(e)}"
...
def insert_before_symbol(
    ...
    try:
        ...
        # Insert the new code
        modified_lines = lines[:insertion_line] + new_lines + [""] + lines[insertion_line:]
        modified_content = '\n'.join(modified_lines)
        ...
        return f"Error inserting code before symbol: {str(e)}"
...
def insert_after_symbol(
    ...
    try:
        ...
        modified_lines = lines[:insertion_point] + [""] + new_lines + lines[insertion_point:]
        modified_content = '\n'.join(modified_lines)
        ...
        return f"Error inserting code after symbol: {str(e)}"
...
def _find_symbol_definition(content: str, symbol_name: str, symbol_type: str) -> Optional[Dict[str, Any]]:
    ...
    for i, line in enumerate(lines):
        ...
        # Python patterns
        if symbol_type == 'function' and line_stripped.startswith(f'def {symbol_name}('):
        ...
        elif symbol_type == 'class' and line_stripped.startswith(f'class {symbol_name}'):
            end_line = _find_symbol_end(lines, i, _get_line_indentation(line))
            return {
                'start_line': i,
                'end_line': end_line,
...
def _find_symbol_line(content: str, symbol_name: str, symbol_type: str) -> Optional[int]:
    """Find the line number where a symbol is defined"""
    lines = content.splitlines()
    
    for i, line in enumerate(lines):
        if symbol_type == 'function' and f'def {symbol_name}(' in line:
            return i
        elif symbol_type == 'class' and f'class {symbol_name}' in line:
            return i
        elif symbol_type == 'variable' and f'{symbol_name} =' in line:
            return i
...
def _get_project_files(project_root: Path) -> List[Path]:
    ...
    for file_path in project_root.rglob("*"):
        if (file_path.is_file() and 
            ...
            not any(part in ['.git', 'node_modules', '__pycache__', '.venv', 'build', 'dist'] 
...
def _smart_rename_symbol(content: str, old_name: str, new_name: str, symbol_type: str) -> tuple[str, int]:
