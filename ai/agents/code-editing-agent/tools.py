...
def _find_symbol_definition(
    ...
    for i, line in enumerate(lines):
        ...
        # Python patterns
        if (symbol_type == "function" and line_stripped.startswith(
            f"def {symbol_name}("
        )) or (symbol_type == "class" and line_stripped.startswith(
            f"class {symbol_name}"
        )):
...
def _find_symbol_line(
    ...
    for i, line in enumerate(lines):
        if (symbol_type == "function" and f"def {symbol_name}(" in line) or (symbol_type == "class" and f"class {symbol_name}" in line):
            return i
