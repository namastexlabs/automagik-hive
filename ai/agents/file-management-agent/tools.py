"""
File Management Agent Tools - Safe File Operations and Content Management

Extracted and adapted from Serena's file operation capabilities for multi-agent architecture.
Focuses on secure file system operations, content manipulation, and data integrity.
"""
    ...
    try:
        ...
        try:
            ...
            for alt_encoding in ['latin-1', 'cp1252', 'utf-16']:
                try:
                    content = file_path.read_text(encoding=alt_encoding, errors='replace')
                    break
                except UnicodeDecodeError:
                    continue
            else:
...
def search_for_pattern(
    ...
    try:
        ...
        if file_pattern == "*":
            extensions = ['.py', '.js', '.ts', '.java', '.cpp', '.c', '.h', '.rb', '.go', '.rs', '.txt', '.md']
        ...
        for file_path in project_root.rglob("*"):
            ...
            if any(part in ['.git', 'node_modules', '__pycache__', '.venv', 'build', 'dist', '.pytest_cache']
                ...
                for line_num, line in enumerate(lines, 1):
                    ...
                    if found_match:
                        ...
                        match_info = {
                            ...
                            'line': line_num,
                            'content': line.strip(),
                            'context': '\n'.join(f"{start_line + i + 1:4d}: {ctx_line}"
        ...
        if not matches:
            ...
            return (f"No matches found for {search_type} pattern '{pattern}' "
                   f"in {files_searched} files searched")
        
        # Format results
        search_type = "regex" if use_regex else "text"
        output = [f"🔍 Search results for {search_type} pattern '{pattern}'"]
        output.append(f"Found {len(matches)} match(es) in {files_searched} files searched:\n")
...
def delete_lines(
    ...
    try:
        ...
        return f"Error deleting lines from '{relative_path}': {str(e)}"
...
def replace_lines(
    ...
    try:
        ...
        return f"Error replacing lines in '{relative_path}': {str(e)}"
...
def insert_at_line(
    ...
    try:
        ...
        if insert_idx < 0:
            return f"Error: line_number must be >= 1"
        ...
        return f"Error inserting content in '{relative_path}': {str(e)}"
...
def _should_include_item(item: Path, show_hidden: bool, file_types: Optional[List[str]]) -> bool:
    ...
    skip_dirs = {'.git', 'node_modules', '__pycache__', '.venv', 'build', 'dist', '.pytest_cache'}
