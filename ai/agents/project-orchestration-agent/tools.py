...
def onboarding(
    ...
    try:
        ...
        return f"Error during project onboarding: {str(e)}"
...
def check_onboarding_performed() -> str:
    ...
    try:
        ...
        if "project_onboarding" in memories:
            ...
            try:
                if "Onboarding Date:" in onboarding_memory:
                    timestamp_line = [line for line in onboarding_memory.split('\n') 
                                    if 'Onboarding Date:' in line][0]
        ...
        return f"Error checking onboarding status: {str(e)}"
...
def activate_project(
    ...
    try:
        ...
        config_files = []
        for config_file in ['package.json', 'requirements.txt', 'Cargo.toml', 'pom.xml', 'build.gradle', 'pyproject.toml']:
    ...
    try:
        ...
        for item in project_root.rglob("*"):
            if item.is_file():
                ...
                if item.suffix in ['.py', '.js', '.ts', '.java', '.cpp', '.c', '.h', '.rb', '.go', '.rs']:
                    structure['languages_detected'].append(item.suffix)
                if item.name in ['README.md', 'package.json', 'requirements.txt', 'Cargo.toml', 'pom.xml']:
                    structure['key_files'].append(str(item.relative_to(project_root)))
            elif item.is_dir() and not any(skip in item.parts for skip in ['.git', 'node_modules', '__pycache__']):
                if len(structure['directories']) < 10:  # Limit output
                    structure['directories'].append(str(item.relative_to(project_root)))
        
        # Remove duplicates and sort
        structure['languages_detected'] = sorted(list(set(structure['languages_detected'])))
        ...
        config_info['agent_capabilities'] = {
            'project_management': ['onboarding', 'activation', 'configuration', 'memory_management'],
            'available_agents': ['code-understanding-agent', 'file-management-agent', 'code-editing-agent'],
            'coordination_modes': ['workflow_orchestration', 'task_delegation', 'result_synthesis']
        }
        ...
        if structure['languages_detected']:
            output.append(f"   💻 Languages: {', '.join(structure['languages_detected'])}")
        ...
        output.append(f"   🔄 Available Agents: {len(config_info['agent_capabilities']['available_agents'])}")
        ...
        return f"Error getting current configuration: {str(e)}"
...
def read_memory(memory_name: str) -> str:
    ...
    try:
        ...
        if not memory_dir.exists():
            return f"Memory store not initialized. No memories available."
        
        memory_file = memory_dir / f"{memory_name}.md"
        
        if not memory_file.exists():
            available_memories = [f.stem for f in memory_dir.glob("*.md")]
            available_list = ", ".join(available_memories) if available_memories else "none"
        ...
        return f"Error reading memory '{memory_name}': {str(e)}"
...
def write_memory(memory_name: str, content: str) -> str:
    ...
    try:
        ...
        safe_name = "".join(c if c.isalnum() or c in '._-' else '_' for c in memory_name)
        ...
        return f"Error writing memory '{memory_name}': {str(e)}"
...
def list_memories() -> str:
    ...
    try:
        ...
        memories = []
        for memory_file in sorted(memory_files, key=lambda f: f.stat().st_mtime, reverse=True):
            stat = memory_file.stat()
            size = stat.st_size
...
def delete_memory(memory_name: str) -> str:
    ...
    try:
        ...
        return f"Error deleting memory '{memory_name}': {str(e)}"
...
def _analyze_dependencies(project_root: Path) -> Dict[str, Any]:
    ...
    if package_file.exists():
        try:
            ...
            dep_count = len(content.get('dependencies', {})) + len(content.get('devDependencies', {}))
...
def _analyze_testing_setup(project_root: Path) -> Dict[str, Any]:
    ...
    test_configs = ['pytest.ini', 'tox.ini', 'jest.config.js', 'karma.conf.js', 'mocha.opts']
    found_configs = []
...
def _generate_recommendations(analysis: Dict[str, Any]) -> List[str]:
    ...
    if 'JavaScript' in analysis['languages_detected'] or 'TypeScript' in analysis['languages_detected']:
        if 'npm/Node.js' not in analysis['build_systems']:
            recommendations.append("Consider initializing with 'npm init' for dependency management")
    
    # General recommendations
    if 'README.md' not in str(analysis['key_files']):
        recommendations.append("Consider adding a README.md file to document the project")
    
    if not analysis['testing_setup']:
        recommendations.append("Consider setting up automated testing for better code quality")
...
{', '.join(analysis['languages_detected']) if analysis['languages_detected'] else 'None detected'}

## Build Systems & Frameworks
{chr(10).join(f"- {bs}" for bs in analysis['build_systems']) if analysis['build_systems'] else '- None detected'}

## Key Files Identified
{chr(10).join(f"- {kf}" for kf in analysis['key_files']) if analysis['key_files'] else '- None found'}

## Dependencies Analysis
{chr(10).join(f"- {lang}: {info['count']} dependencies ({info['file']})" 
           for lang, info in analysis['dependencies'].items()) if analysis['dependencies'] else '- No dependency files found'}

## Testing Setup
{chr(10).join(f"- Test directories: {', '.join(analysis['testing_setup'].get('test_directories', []))}" +
           f"- Config files: {', '.join(analysis['testing_setup'].get('config_files', []))}"
           if analysis['testing_setup'] else '- No testing setup detected')}

## Recommendations
{chr(10).join(f"- {rec}" for rec in analysis['recommendations']) if analysis['recommendations'] else '- Project setup looks good'}
