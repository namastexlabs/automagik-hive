...
def finalize_conversation(
    ...
    """
    Complete conversation finalization process.
    
    This is a convenience tool that combines typification, protocol retrieval,
    and farewell message sending into a single operation.
    ...
    Returns:
        Final farewell message with protocol
    """
    
    logger.info(f"🎯 Starting complete conversation finalization for session {session_id}")
    
    try:
        # Step 1: Trigger typification workflow
        typification_result = trigger_conversation_typification_workflow(
            session_id=session_id,
        ...
        # Extract protocol ID from typification result
        protocol_id = None
        if "Protocolo:" in typification_result:
            protocol_id = typification_result.split("Protocolo: ")[1].split()[0]
        
        if not protocol_id:
            logger.warning("No protocol ID found in typification result")
            return "⚠️ Atendimento finalizado, mas protocolo não disponível"
        
        # Step 2: Send farewell message
        farewell_message = send_farewell_message(
            protocol_id=protocol_id,
            customer_name=customer_name,
            message_type="grateful"
        )
        
        logger.info(f"✅ Conversation finalization completed successfully")
        return farewell_message
        
    except Exception as e:
        logger.error(f"❌ Conversation finalization failed: {str(e)}")
        return f"❌ Erro na finalização: {str(e)}"