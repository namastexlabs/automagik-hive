...
def trigger_conversation_typification_workflow(
    ...
    try:
        ...
        if not is_workflow_registered("conversation-typification"):
            logger.debug(
                "🤖 Conversation typification workflow not available - graceful handling"
            )
        ...
        if hasattr(final_result, "metadata") and final_result.metadata:
            ...
            if protocol_id:
                logger.info(
                    f"✅ Typification completed successfully with protocol: {protocol_id}"
                )
        ...
        logger.error(f"❌ Typification workflow failed: {e!s}")
        return f"❌ Erro na tipificação: {e!s}"
...
def send_farewell_message(
    ...
    try:
        ...
        elif message_type == "grateful":
            farewell = (
                f"Fico feliz em ter ajudado! {protocol_message}. Agradecemos!"
            )
        elif message_type == "professional":
        ...
        logger.info("✅ Farewell message created successfully")
        return farewell

    except Exception as e:
        logger.error(f"❌ Failed to create farewell message: {e!s}")
        return f"❌ Erro ao criar mensagem de despedida: {e!s}"
