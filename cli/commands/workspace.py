"""Unified Workspace Management Module.

Consolidates duplicate workspace functionality from workspace.py and workspace_manager.py
into a single, comprehensive workspace management system.

Features:
- Interactive workspace creation and initialization
- Workspace validation and health diagnostics
...
class UnifiedWorkspaceManager:
    """Unified workspace management with initialization, validation, and server operations."""
    ...
    def start_workspace_server(self, workspace_path: str) -> bool:
        """Start workspace server with dependency auto-detection and unified validation.

        Combines the server startup logic from workspace.py with the dependency
        detection from workspace_manager.py.
        ...
        try:
            ...
            if not self._validate_workspace_existence():
                return False
            ...
            # Step 3: Auto-detect missing dependencies
            missing_deps = self._detect_missing_dependencies()
            ...
            # Step 4: Validate PostgreSQL connection
    ...
    def _validate_workspace_existence(self) -> bool:
        """Validate workspace directory and required files (from workspace.py)."""
        ...
        if missing_files:
            print(
                f"❌ Workspace not initialized. Missing files: {', '.join(missing_files)}"
            )
    ...
    def _start_server(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                print(
                    "💡 Make sure you're running from a valid Automagik Hive workspace"
                )
                return False
    ...
    def _check_python_dependencies_missing(self) -> bool:
        ...
        dependencies = ["agno", "fastapi", "uvicorn"]
...
WorkspaceManager = UnifiedWorkspaceManager
WorkspaceCommands = UnifiedWorkspaceManager  # For test compatibility