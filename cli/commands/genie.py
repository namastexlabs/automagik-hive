...
def genie_serve_cmd(workspace_path: str | Path | None = None) -> bool:
    """Genie serve command stub function."""
    return True


def genie_status_cmd(workspace_path: str | Path | None = None) -> dict[str, Any]:
    """Genie status command stub function."""
    return {"status": "running", "healthy": True}


def genie_stop_cmd(workspace_path: str | Path | None = None) -> bool:
