...
class DockerService:
    ...
    def get_docker_status(self) -> Tuple[bool, str, Optional[str]]:
        ...
        try:
            ...
            return False, f"Docker execution error: {str(e)}", None

    def _get_docker_command(self) -> Optional[str]:
    ...
    def get_docker_daemon_status(self) -> Tuple[bool, str, Optional[Dict]]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if "Cannot connect to the Docker daemon" in error_msg:
                    ...
                    return False, "Docker daemon access denied - check permissions", None
                else:
                    return False, f"Docker daemon error: {error_msg}", None
                    
        except subprocess.TimeoutExpired:
            return False, "Docker daemon check timed out - daemon may be unresponsive", None
        except (FileNotFoundError, OSError) as e:
            return False, f"Docker daemon check failed: {str(e)}", None
    ...
    def _parse_docker_info(self, info_output: str) -> Dict[str, str]:
        ...
        for line in info_output.split('\n'):
            ...
            if ':' in line:
                ...
                # Capture key metrics
                if key in ['Server Version', 'Storage Driver', 'Operating System', 'Architecture']:
                    info[key] = value
                elif key == 'Containers':
    ...
    def get_platform_specific_installation_guide(self) -> Dict[str, str]:
        ...
        if machine in ["arm64", "aarch64"] and system == "linux":
            current_guide["notes"] += " | ARM64 architecture detected - ensure ARM-compatible images"
        elif machine in ["arm64", "aarch64"] and system == "darwin":
            current_guide["notes"] += " | Apple Silicon detected - Docker Desktop includes ARM support"
            
        return current_guide

    def detect_wsl_environment(self) -> Tuple[bool, Optional[str]]:
    ...
    def get_docker_compose_version(self) -> Tuple[bool, Optional[str], Optional[str]]:
    ...
    def comprehensive_docker_check(self) -> Dict[str, any]:
        ...
        compose_available, compose_version, compose_type = self.get_docker_compose_version()
        ...
        # Installation guide
        check_results["installation_guide"] = self.get_platform_specific_installation_guide()
    ...
    def validate_workspace_after_creation(self, workspace_path: Path) -> Tuple[bool, List[str], List[str]]:
            ...
            if file_path.exists():
                ...
                if filename == ".env":
                    ...
                    if self.validate_compose_file(str(workspace_path)):
                        success_messages.append(f"   • Docker Compose syntax valid")
                    else:
                        error_messages.append(f"❌ Docker Compose syntax validation failed")
        ...
        try:
            ...
            if env_file.exists():
                ...
                if file_mode == "600":
                    ...
                    error_messages.append(f"⚠️ .env file permissions: {file_mode} (should be 600)")
        ...
        if data_path.exists():
            try:
                ...
                test_file.touch()
                test_file.unlink()
                success_messages.append("✅ Data directory has proper write permissions")
            except PermissionError:
                error_messages.append("❌ Data directory permission issues - may need chown")
            except Exception as e:
                error_messages.append(f"⚠️ Could not verify data directory permissions: {e}")
