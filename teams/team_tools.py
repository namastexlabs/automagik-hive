"""
Shared tools and utilities for PagBank teams
Agent E: Team Framework Development
Custom tools implementing PagBank-specific functionality
"""
...
def check_security_alert(alert_type: str, data: Dict[str, Any]) -> Dict[str, Any]:
    """Check for security alerts in team operations
...
def get_team_tools(team_name: str) -> List[Callable]:
    """Get appropriate tools for a specific team"""
    # All teams get validation tool
    tools = [TEAM_TOOLS["validation"]]
    
    # Team-specific tools
    if team_name in ["cartoes", "conta_digital"]:
        tools.append(TEAM_TOOLS["security"])
    
    if team_name in ["credito", "investimentos"]:
        tools.append(TEAM_TOOLS["calculator"])
    
    if team_name == "seguros":
        tools.append(TEAM_TOOLS["calculator"])  # For premium calculations
