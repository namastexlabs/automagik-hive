...
check_port_conflicts() {
    ...
    MAIN_PORTS=$(docker compose -f docker-compose.yml config | grep -E "^\s*-\s*\"[0-9]+:" | sed 's/.*"\([0-9]*\):.*/\1/' | sort)
    GENIE_PORTS=$(docker compose -f docker-compose-genie.yml config | grep -E "^\s*-\s*\"[0-9]+:" | sed 's/.*"\([0-9]*\):.*/\1/' | sort)
    AGENT_PORTS=$(docker compose -f docker-compose-agent.yml config | grep -E "^\s*-\s*\"[0-9]+:" | sed 's/.*"\([0-9]*\):.*/\1/' | sort)
...
validate_network_isolation() {
    ...
    MAIN_NETWORKS=$(docker compose -f docker-compose.yml config | grep -A 5 "networks:" | grep -v "networks:" | grep -E "^\s+[a-z_]+" | awk '{print $1}' | tr -d ':')
    GENIE_NETWORKS=$(docker compose -f docker-compose-genie.yml config | grep -A 5 "networks:" | grep -v "networks:" | grep -E "^\s+[a-z_]+" | awk '{print $1}' | tr -d ':')
    AGENT_NETWORKS=$(docker compose -f docker-compose-agent.yml config | grep -A 5 "networks:" | grep -v "networks:" | grep -E "^\s+[a-z_]+" | awk '{print $1}' | tr -d ':')
...
validate_compose_file "docker-compose.yml" "Main Workspace Services"
validate_compose_file "docker-compose-genie.yml" "Genie Consultation Container"
validate_compose_file "docker-compose-agent.yml" "Agent Development Container"

# Validate Dockerfiles
validate_dockerfile "Dockerfile" "Main Application Dockerfile"
validate_dockerfile "Dockerfile.genie" "Genie All-in-One Dockerfile"
validate_dockerfile "Dockerfile.agent" "Agent All-in-One Dockerfile"
...
echo -e "\n${BLUE}Next Steps:${NC}"  
echo "1. Test containers: docker compose up -d"
echo "2. Verify health checks: docker compose ps"
echo "3. Test Genie container: docker compose -f docker-compose-genie.yml up -d"
echo "4. Test Agent container: docker compose -f docker-compose-agent.yml up -d"
