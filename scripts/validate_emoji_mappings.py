...
class EmojiMappingValidator:
    ...
    def __init__(self, emoji_config_path: Optional[str] = None):
        """
        ...
        for file_path in file_paths:
            ...
            if normalized_path.endswith('.py'):
                try:
                    ...
                    service_matches = self.resource_patterns['new_service'].findall(content)
    ...
    def _check_missing_mappings(self, detected_resources: Dict[str, Set[str]]) -> Dict[str, Set[str]]:
        """
    ...
    def _generate_mapping_suggestions(self, missing_mappings: Dict[str, Set[str]]) -> None:
    ...
    def format_violations(self, output_format: str = 'text') -> str:
        ...
        if self.violations:
            ...
            for i, violation in enumerate(self.violations, 1):
                ...
                output.append(f"{i}. Missing emoji mapping for {resource_type}: {resource}")
                output.append(f"   Suggested emoji: {suggestion}")
                output.append(f"   Add to {config_path}:")
                output.append(f"   resource_types:")
...
def main():
    ...
    parser.add_argument(
        '--config',
        help='Path to emoji_mappings.yaml file'
    )
